...
<Note>
  The proxy's type, IP version, protocol and location all come from the offer —
  there are no request fields for them. Pick where you want the proxy by
  choosing the right `offer_id`: the catalog exposes `country_code`, `city` and
  `region` on every offer, and offers whose `city` is `null` let the provider
  choose within the country. See [List proxy offers](/api/catalog/list-offers).
</Note>
...
<ParamField body="period_days" type="integer" default="30">
  Rental period in days — a legacy alternative to `tarification_index`. It
  must exactly match one of the offer's `options` periods (`30`, `60`, …,
  `360` for residential/datacenter); the server resolves it to that option
  and returns `422` when no option matches. When `tarification_index` is
  sent, `period_days` (if present) must match the chosen option's
  `period_days` exactly.
</ParamField>

<ParamField body="tarification_index" type="integer">
  Index into the offer's `options` array — the primary way to pick a
  price/period for **every** offer kind. The order is charged the chosen
  option's price for its full period (multiplied by `quantity`). Required
  for mobile rotating offers, which sell only pre-registered tariff
  triples.
</ParamField>
