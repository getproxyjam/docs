...
`country_code`, `mode`, `ip_version`, `is_available`, `currency`, and
`options` (JSON-encoded list of tariff variants; each variant carries a
`price` in the row's `currency`). Use `id` as `offer_id` in
`create_proxy_order`.
